    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <Input
          ...
          className={`h-10 text-sm flex-1 rounded-xl border-border/50 focus:border-primary transition-[border-color,background-color,box-shadow,color] duration-200 ${validationError ? 'border-destructive focus-visible:ring-destructive' : ''}`}
          name="person-name"
          autoComplete="off"
        ...
        {showButton && (
          <Button 
            ...
            className={`h-10 px-4 rounded-xl btn-float transition-[background-color,box-shadow,transform,color] duration-300 font-medium ${showSuccess ? 'bg-success hover:bg-success/90 success-pulse' : 'bg-gradient-to-br from-primary to-primary/90 hover:from-primary-600 hover:to-primary/80 text-white'}`}
          >
            {showSuccess ? '✓' : 'Add'}
          </Button>
        )}
